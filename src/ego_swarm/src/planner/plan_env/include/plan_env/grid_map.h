...
struct MappingParameters
{
  ...
  // virtual_ground: 镜像 virtual_ceil, 把 z<=virtual_ground_height_ 的整段 inflate 全部填占据,
  // 防止 B 样条优化器的 {p,v} 逃逸把控制点向 -Z 推。
  // 启用条件: 飞机 z 必须 > virtual_ground_enable_height_, 避免起飞时把自己困在 inflate 里。
  double virtual_ground_height_;
  double virtual_ground_enable_height_;
  bool show_occ_time_;
...
private:
  ...
  MappingData md_;
  vector<Eigen::Vector3i> inflation_offsets_;
  ...
  void applyVirtualBoundsToInflatedMap();

  inline void inflatePoint(const Eigen::Vector3i &pt, vector<Eigen::Vector3i> &pts);
  ...
  rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr ceil_marker_pub_;
...
inline int GridMap::getInflateOccupancy(Eigen::Vector3d pos)
{
  ...
  // 浮点精度兜底:isInMap 用 pos<boundary+1e-4 比较,但 floor 在边界上可能算出
  // id == map_voxel_num,toAddress 越界访问会 SIGSEGV。多项式初始轨迹的控制点
  // 经常 overshoot 到边界附近,这里保守把越界视为 occupied(-1)。
...
inline void GridMap::inflatePoint(const Eigen::Vector3i &pt, vector<Eigen::Vector3i> &pts)
{
  pts.clear();
  pts.reserve(inflation_offsets_.size());
