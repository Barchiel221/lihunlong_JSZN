...
def generate_launch_description():
    ...
    obstacles_inflation_z = LaunchConfiguration('obstacles_inflation_z', default=-1.0)
    dist0 = LaunchConfiguration('dist0', default=0.5)
    ...
    emergency_time = LaunchConfiguration('emergency_time', default=1.0)

    # 方案三新增: 默认全部关闭/无影响, 实飞 launch 通过 launch_arguments 覆盖。
    virtual_ground_height = LaunchConfiguration('virtual_ground_height', default=-10.0)
    virtual_ground_enable_height = LaunchConfiguration('virtual_ground_enable_height', default=1e9)
    lambda_z = LaunchConfiguration('lambda_z', default=0.0)
    # 旁支修复: 原来 dict 里 virtual_ceil_height 是写死 1.0, 外层 launch_arguments 传值被默默丢弃。
    # 现在改成读 LaunchConfiguration, 实飞 launch 的 'virtual_ceil_height': '1.5' 才能真正生效。
    virtual_ceil_height = LaunchConfiguration('virtual_ceil_height', default=1.0)
    ...
    obstacles_inflation_arg = DeclareLaunchArgument('obstacles_inflation', default_value=obstacles_inflation, description='XY obstacle inflation radius')
    obstacles_inflation_z_arg = DeclareLaunchArgument('obstacles_inflation_z', default_value=obstacles_inflation_z, description='Z obstacle inflation radius')
    dist0_arg = DeclareLaunchArgument('dist0', default_value=dist0, description='Optimizer soft clearance')
    ...
    emergency_time_arg = DeclareLaunchArgument('emergency_time', default_value=emergency_time, description='Emergency stop lookahead time')
    virtual_ground_height_arg = DeclareLaunchArgument('virtual_ground_height', default_value=virtual_ground_height, description='Virtual ground occupied height')
    virtual_ground_enable_height_arg = DeclareLaunchArgument('virtual_ground_enable_height', default_value=virtual_ground_enable_height, description='Enable virtual ground above this drone height')
    lambda_z_arg = DeclareLaunchArgument('lambda_z', default_value=lambda_z, description='Altitude soft penalty weight')
    virtual_ceil_height_arg = DeclareLaunchArgument('virtual_ceil_height', default_value=virtual_ceil_height, description='Virtual ceiling occupied height')
    ...
    ego_planner_node = Node(
        ...
        parameters=[
            ...
            {'grid_map/obstacles_inflation_z': obstacles_inflation_z},
            ...
            # virtual_ground: 默认关闭 (-10.0). 实飞 launch 会覆盖成 0.45 / enable=0.55, 配合方案三阻止 -Z 下钻。
            {'grid_map/virtual_ground_height': virtual_ground_height},
            {'grid_map/virtual_ground_enable_height': virtual_ground_enable_height},
            # 1.3: 卡在 virtual_ceil_height=1.5 对应 inflate 起始层 (z=1.39) 下面,
            # 避免把 16m×16m 的 vc 填充层画进 occupancy_inflate, 否则 RViz 里像一块巨大天花板。
            {'grid_map/visualization_truncate_height': 1.3},
            ...
            # altitude penalty: 默认 0 = 不生效. 实飞 launch 覆盖, 让优化器偏好水平 ±Y 侧绕。
            {'optimization/lambda_z': lambda_z},
    ...
    ld.add_action(obstacles_inflation_z_arg)
    ld.add_action(dist0_arg)
    ld.add_action(local_update_range_x_arg)
    ld.add_action(local_update_range_y_arg)
    ld.add_action(local_update_range_z_arg)
    ld.add_action(emergency_time_arg)
    ld.add_action(virtual_ground_height_arg)
    ld.add_action(virtual_ground_enable_height_arg)
    ld.add_action(lambda_z_arg)
