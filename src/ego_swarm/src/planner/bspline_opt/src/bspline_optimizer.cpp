...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(rclcpp::Node::SharedPtr node)
  {
    ...
    // altitude penalty: 默认 0 = 关闭, 不影响旧 launch
    node->declare_parameter("optimization/lambda_z", 0.0);
    ...
    node->get_parameter("optimization/lambda_z", lambda_z_);
    z_ref_ = 0.0;  // 占位, planner_manager 会在每次 replan 前 setAltitudeRef() 注入
  ...
  // altitude penalty: 对每个内部控制点惩罚 (z - z_ref_)^2,
  // 让 -Z 下钻 / +Z 抬升都被定价, 使优化器更偏好水平 ±Y 侧绕。
  // 仅作用于"被优化的"控制点 [order_, q.cols()-order_), 与 calcSmoothnessCost 的写法保持一致 (端点固定)。
  void BsplineOptimizer::calcAltitudeCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    cost = 0.0;
    int end_idx = q.cols() - order_;
    for (int i = order_; i < end_idx; ++i)
...
// #define USE_SECOND_CLEARENCE_CHECK
  ...
  void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    double f_smoothness, f_distance, f_feasibility /*, f_mov_objs*/, f_swarm, f_terminal, f_altitude;
    ...
    Eigen::MatrixXd g_altitude = Eigen::MatrixXd::Zero(3, cps_.size);
    ...
    f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ * f_swarm + lambda2_ * f_terminal + lambda_z_ * f_altitude;
    // f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ * f_mov_objs;
    // printf("origin %f %f %f %f\n", f_smoothness, f_distance, f_feasibility, f_combine);

    Eigen::MatrixXd grad_3D = lambda1_ * g_smoothness + new_lambda2_ * g_distance + lambda3_ * g_feasibility + new_lambda2_ * g_swarm + lambda2_ * g_terminal + lambda_z_ * g_altitude;
