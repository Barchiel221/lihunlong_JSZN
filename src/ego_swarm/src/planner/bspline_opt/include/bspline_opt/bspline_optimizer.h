...
namespace ego_planner
{
  ...
  private:
    ...
    // altitude penalty: 软约束 z 接近 z_ref_, 让"-Z 下钻"在 cost 上变贵, 引导优化器走 ±Y 侧绕。
    // z_ref_ 由 planner_manager 每次 reboundReplan 时 setAltitudeRef() 注入 (通常用 start_pt.z 或飞行参考高度)。
    double lambda_z_;              // altitude penalty weight
    double z_ref_;                 // reference altitude
...
#define INIT_min_ellip_dist_ 123456789.0123456789
    ...
    void calcAltitudeCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient);
