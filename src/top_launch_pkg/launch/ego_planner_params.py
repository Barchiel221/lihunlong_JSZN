...
def ego_advanced_launch_arguments(max_vel='0.6', max_acc='0.5',
    ...
    args = {
        ...
        # 环高=1.20: 设 1.5 时峰值上限≈1.27 只剩 7cm, dist0 软墙下压导致 ego 不肯顶到环高
        # (bag 132204 撞环复盘)。抬到 1.7 → 峰值≈1.47, 给 1.20 环留 ~0.27m 呼吸。
        # 前提: 赛场竖直净空 ≥1.7m (已确认)。
        'virtual_ceil_height': '1.7',
        ...
        'virtual_ground_enable_height':  '0.30',
        # altitude penalty: lambda_z 越大越偏好水平飞行。5.0 把爬升后置/压死, 飞机到环x时仍偏低
        # (bag 132204: z@x=6 仅~1.1, 实飞 0.81 撞环底)。降到 2.5 让爬升前置, 早到环高。
        'lambda_z':                      '2.5',
