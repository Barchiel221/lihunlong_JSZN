...
- PX4 经 Micro-XRCE-DDS-Agent 走 `/dev/ttyS6`（因此 `uart_to_stm32` 被注释停用，二者冲突）
...
- **当前停用**（launch 中注释掉）：`uart_to_stm32`、`pid_control_pkg`、`activity_control_pkg`、`serial_comm`

## 关键约束与已知坑

- **`ego_replay.launch.py` 的 planner 参数必须与 `ego_real_flight.launch.py` 的 `ego_advanced` 段保持一致**——改任何一边都要同步另一边（两个文件里都有此注释）。
...
- 调参（inflation / virtual_ground / virtual_ceil / lambda_z 等）的取值依据都写在 `ego_real_flight.launch.py` 的行内注释里，且多数来自具体 bag 复盘；改参数前先读注释和对应 `bag_analysis/FINDINGS_*.md`。
